...
class BaseIteration(object):
	...
	def process_results(self):
		...
		if self.stage == len(self.budgets):
			predict_budgets = 1
		else:
			predict_budgets = sum(self.budgets[self.stage:])
		if self.stat:
			losses = np.array([self.data[cid].results[budget]['acc_record'] for cid in config_ids])
		else:
			losses = np.array([self.data[cid].results[budget]['loss'] for cid in config_ids])

		advance = self._advance_to_next_stage(config_ids, losses, predict_budgets)
